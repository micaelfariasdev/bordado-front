...
import { Avatar, IconButton, Menu, MenuItem, Typography, Box, CircularProgress, Button, Divider } from "@mui/material"
...
export default function UserMenu() {
    ...
    useEffect(() => {
        const fetchMe = async () => {
            try {
                const resp = await axios.get("http://localhost:3000/api/whatsapp/me")
                setUser(resp.data)
            } catch (error) {
            ...
            <Box display="flex" alignItems="center" gap={1}>
                ...
                >
                    <MenuItem className="flex flex-col" >
                        ...
                        <Typography className="italic text-[12px]">{formatarTelefone(user.Número)}</Typography>
                    </MenuItem>
                    <Divider sx={{ mb: 1 }} />
                    <MenuItem onClick={handleLogout} disabled={loading}>

                        {loading ? <CircularProgress size={20} /> : <LogoutIcon fontSize="small" sx={{ mr: 1 }} />}
                        Sair
                    </MenuItem>
                </Menu>
            </Box>
