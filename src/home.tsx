...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (pedidos.length > 0 && profile?.logged) {
      ...
      axios.post("http://localhost:3000/api/whatsapp/historico", { numeros: pedidosIds })
    }
    ...
    <div className="flex flex-col h-screen">
      <header className="bg-blue-950 h-24 flex items-center justify-center">
        <Stack direction="row" spacing={2}>
          ...
          {/* <ActionMenu
            avatar={<Avatar alt={profile?.user} src={profile?.profilePic} sx={{ width: 56, height: 56 }} />}
          ...
          <PedidoCard
            ...
            onSendMessage={async (to, message) => {
              if (wsRef.current?.readyState === WebSocket.OPEN) {
                wsRef.current.send(JSON.stringify({ type: "send-message", to, message }))
              }
            }}
          />
        ))}
